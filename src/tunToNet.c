...
void tunToNetNoBlastFlavor (struct contextSimplemux* context)
{
  ...
  if ( drop_packet == false ) {
    ...
    if (context->flavor == 'N') {
      ...
      // one-byte separator
      if (context->sizePacketsToMultiplex[context->numPktsStoredFromTun] < maximum_packet_length ) {
      ...
      // two-byte separator
      else if (context->sizePacketsToMultiplex[context->numPktsStoredFromTun] < limit_length_two_bytes ) {
      ...
      // print the two bytes of the separator
        ...
        if(debug>0) {
          ...
          // first byte
          FromByte(context->separatorsToMultiplex[context->numPktsStoredFromTun][0], bits);
          do_debug(2, " Mux separator of 3 bytes. Length: 0x%02x (", context->separatorsToMultiplex[context->numPktsStoredFromTun][0]);
          PrintByte(2, 8, bits);
          do_debug(2, ")");
    ...
    if ((context->numPktsStoredFromTun == context->limitNumpackets) || (context->sizeMuxedPacket > context->sizeThreshold) || (time_difference > context->timeout )) {
      ...
      switch (context->mode) {
        ...
        if ( context->log_file != NULL ) {
          switch (context->mode) {
            case UDP_MODE:
              fprintf (context->log_file, "%"PRIu64"\tsent\tmuxed\t%i\t%"PRIu32"\tto\t%s\t%d\t%i", GetTimeStamp(), context->sizeMuxedPacket + IPv4_HEADER_SIZE + UDP_HEADER_SIZE, context->tun2net, inet_ntoa(context->remote.sin_addr), ntohs(context->remote.sin_port), context->numPktsStoredFromTun);
            break;
            case TCP_CLIENT_MODE:
              fprintf (context->log_file, "%"PRIu64"\tsent\tmuxed\t%i\t%"PRIu32"\tto\t%s\t%d\t%i", GetTimeStamp(), context->sizeMuxedPacket + IPv4_HEADER_SIZE + TCP_HEADER_SIZE, context->tun2net, inet_ntoa(context->remote.sin_addr), ntohs(context->remote.sin_port), context->numPktsStoredFromTun);
            break;
            case NETWORK_MODE:
              fprintf (context->log_file, "%"PRIu64"\tsent\tmuxed\t%i\t%"PRIu32"\tto\t%s\t\t%i", GetTimeStamp(), context->sizeMuxedPacket + IPv4_HEADER_SIZE, context->tun2net, inet_ntoa(context->remote.sin_addr), context->numPktsStoredFromTun);
            break;
          }
          if (context->numPktsStoredFromTun == context->limitNumpackets)
